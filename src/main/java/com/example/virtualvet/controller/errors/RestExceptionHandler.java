...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleResourceAlreadyExistsException(ResourceAlreadyExistsException exception) {
        return new ResponseEntity<>(new ErrorResponse(Instant.now(), exception.getMessage(), HttpStatus.CONFLICT),
                                    HttpStatus.CONFLICT);
    }
    ...
    private record ErrorResponse(Instant timestamp, String message, HttpStatus status) {
    }
}
